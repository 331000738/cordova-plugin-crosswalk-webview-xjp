...
# cordova-plugin-crosswalk-webview-xjp
...
```
$ cordova plugin add https://github.com/waitaction/cordova-plugin-crosswalk-webview-xjp.git
```
...
Note that you might have to run `cordova clean` before building, if you previously built the app without cordova-plugin-crosswalk-webview-xjp. Also, manually uninstall the app from the device/emulator before attempting to install the crosswalk-enabled version.
...
You can try out a different Crosswalk version by specifying certain variables while installing the plugin, or by changing the value of `xwalkVersion` in your `config.xml` after installing the plugin. Some examples:
    ...
    cordova plugin add cordova-plugin-crosswalk-webview-xjp --variable XWALK_VERSION="org.xwalk:xwalk_core_library:14+"
    cordova plugin add cordova-plugin-crosswalk-webview-xjp --variable XWALK_VERSION="xwalk_core_library:14+"
    cordova plugin add cordova-plugin-crosswalk-webview-xjp --variable XWALK_VERSION="14+"
    cordova plugin add cordova-plugin-crosswalk-webview-xjp --variable XWALK_VERSION="14"
...
You can also use a Crosswalk beta version. Some examples:
    ...
    <!-- These are all equivalent -->
    cordova plugin add cordova-plugin-crosswalk-webview-xjp --variable XWALK_VERSION="org.xwalk:xwalk_core_library_beta:14+"
...
You can set [command-line flags](http://peter.sh/experiments/chromium-command-line-switches/) as well:
    ...
    <!-- This is the default -->
    cordova plugin add cordova-plugin-crosswalk-webview-xjp --variable XWALK_COMMANDLINE="--disable-pull-to-refresh-effect"
...
You can use the Crosswalk [shared mode](https://crosswalk-project.org/documentation/shared_mode.html) which allows multiple Crosswalk applications to share one Crosswalk runtime downloaded from the Play Store.
    ...
    <!-- These are all equivalent -->
    cordova plugin add cordova-plugin-crosswalk-webview-xjp  --variable XWALK_MODE="shared"
...
You can also use a Crosswalk beta version on shared mode, e.g.:
    ...
    cordova plugin add cordova-plugin-crosswalk-webview-xjp --variable XWALK_VERSION="org.xwalk:xwalk_shared_library_beta:14+"
...
You can use the Crosswalk [lite mode](https://crosswalk-project.org/documentation/crosswalk_lite.html) which is the Crosswalk runtime designed to be as small as possible by removing less common libraries and features and compressing the APK.
    ...
    <!-- These are all equivalent -->
    cordova plugin add cordova-plugin-crosswalk-webview-xjp  --variable XWALK_MODE="lite"
